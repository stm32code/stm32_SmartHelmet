...
unsigned char MQTT_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"lon\":\"%.6f\",", longitude_sum); 
	strcat(buf, text);
	...
	memset(text, 0, sizeof(text));
	if(Data_init.voice >100){
			sprintf(text, "\"voice\":\"%d\",", 1);

	}else{
			sprintf(text, "\"voice\":\"%d\",", 0);
	}
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"light\":\"%d\",",Data_init.light); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"put_cap\":\"%d\",",device_state_init.Heard_State); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"heard\":\"%d\",", device_state_init.Fall_State); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"warning\":\"%d\"", device_state_init.Warning_State); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);
...
unsigned char Mqtt_Pwd_Info(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"Pwd\":\"%d\"", 0); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);
